...
import { Button } from "@/components/ui/shugar/button-1";
import { Material } from "@/components/ui/shugar/material-1";
import { Input } from "@/components/ui/shugar/input";
import { Select } from "@/components/ui/shugar/select-1";
import { formatInTimeZone, fromZonedTime } from "date-fns-tz";
import { useClickOutside } from "@/components/ui/shugar/use-click-outside";
...
const CalendarCombobox = ({
  ...
  useClickOutside(ref as any, () => setIsOpen(false));
...
export const Calendar = ({
  ...
  const [startTime, setStartTime] = useState<string>(formatInTimeZone(value?.start ? startOfDay(value.start) : startOfDay(new Date()), selectedTimezone, "HH:mm"));
  const [endDate, setEndDate] = useState<string>(formatInTimeZone(value?.end || new Date(), selectedTimezone, "MMM dd, yyyy"));
  const [endTime, setEndTime] = useState<string>(formatInTimeZone(value?.end ? endOfDay(value.end) : endOfDay(new Date()), selectedTimezone, "HH:mm"));
